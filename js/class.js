...
class Bot {
	...
	hasHardMode() {
		return [WORDLE, POLYGONLE].includes(this.type);
	}
	...
	hasScore() {
		return (bot.getCount() == 1) && (!([XORDLE, FIBBLE, SPOTLE].includes(this.type)));
	}

	guessesAllowed() {
		if (this.type == ANTI) return INFINITY;
		return Number(document.getElementById("max-guesses").value);
	}
	...
	getAllPossibleAnswersFrom(list, guessCount) {
		list = filterList(list, false, false, bot.getCount() > 1, true, guessCount);

		if (bot.isFor(XORDLE)) {
			list = xordleFilter(uniqueWordsFrom(list));
		} else if (bot.isFor(POLYGONLE)) {
			list = polygonleFilter(uniqueWordsFrom(list));
		}
	...
	isGuessable(answer) {
		if (this.type == XORDLE && typeof answer == "object") {
			return this.isGuessable(answer.word1) && this.isGuessable(answer.word2);
		}

		return guessable.includes(answer);
	}
	...
	isValidGuess(guess) {
		...
		return guessable.includes(guess);
	}
}
...
function setRowDifferencesWithoutPositions(coloring, row) {
	...
	if (num_correct.options.length > 0) {
		num_correct.options.selectedIndex = correct;
	} else {
		num_correct.innerHTML = `<option value="` + correct + `">` + correct + `</option>`;
	}
	if (num_wrong_spots.options.length > 0) {
		num_wrong_spots.options.selectedIndex = wrong_spots;
	} else {
...
function mostCommonLetters(list) {
	...
	let letters = makeAlphabetArray(word_length+1);
	let checked;
...
function bestWarmleLetters(list) {
	...
	let new_letters = makeAlphabetArray(word_length+1);
...
function calculateAverageBucketSize(guess, answers, min, future_guess) {
	...
	for (let i = 0; i < list_size; i++) {
		...
		if (freq > 0) {
			weighted += (2*freq-1)/list_size/*(freq/list_size)*freq - ((freq-1)/list_size)*(freq-1)*/;
			if (freq > 1) {
				threes -= 1/list_size;
			}
		}
		...
		// if (isFinite(SIZE_FACTOR) && !bot.isFor(ANTI) && (adjusted > min && future_guess || adjusted > min*SIZE_FACTOR)) {
			// return;
		// }
	}
	adjusted = (1-threes)*weighted;
