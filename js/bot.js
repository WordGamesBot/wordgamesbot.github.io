...
function getStartingWords() {
	let guesses = getFirstGuesses(bot_difficulty);
...
function testStartingWords(bot_difficulty = "ultra") {
	let old_difficulty = difficulty;
	difficulty = bot_difficulty;
	let check_list = getStartingWords(bot_difficulty);
	let i = -1;
	...
	let iv = setInterval(function() {
		if (averages.length > i) {
			...
			runBot(check_list[i], bot_difficulty);
		}
...
function runBot(guess, bot_difficulty = "ultra") {
	let old_difficulty = difficulty;
	difficulty = bot_difficulty;
	...
	let iv = setInterval(function() {
		...
		let points = wordleBot(guess, testing_sample[count]);
		...
		if (count >= TEST_SIZE) {
			...
			showResults(guess, TEST_SIZE - missed.length, TEST_SIZE, parseFloat(sum/count).toFixed(3), missed);
			updateWordData(guess, sum, missed);
			printData(newlist, guess, sum, (performance.now() - start_time)/1000);
...
function updateWordData(guess, average, missed) {
	averages.push({word: guess, average: average, wrong_answers: missed});
	averages.sort((a, b) => (a.average > b.average)? 1: ((a.average == b.average)? 0: -1));
	...
	let data = {average: average, wrong_answers: missed};
	...
	newlist[bot.type][index][answerlist+"|"+guesslist] = data;
}
...
function wordleBot(guess, answer) {
	...
	while (attempts <= bot.guessesAllowed()) {
		...
		let final_guesses = getBestGuesses(lists, attempts-1);
